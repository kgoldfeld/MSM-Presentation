...
genTimeDep <- function(id, T, k, Theta, Gamma) {
  ...
  L <- vector(mode = "integer", length = T+1)
  ...
  for (j in (1:T)) {
    ...
    if ( (j %% k) != 0) {
      ...
      L[j+1] <- max(0, L[j] + 150 * A[j + 2 - 5] * (1-A[j + 2 - 6]) + e[j+1])
      W[j+1] <- (L[j+1] > 500)
...
myfit <- glm(A ~ period + Ladj, data=dt[post==0], family=binomial)
